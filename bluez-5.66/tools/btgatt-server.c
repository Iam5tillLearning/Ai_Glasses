...
// 添加在main函数之前
static int send_socket_command(const char *cmd);

#define SOCKET_PATH "/tmp/display_control.sock"  // 与显示控制程序一致
...
#ifdef CUSTOM_SERVER
static void custom_device_read_cb(struct gatt_db_attribute *attrib,
					unsigned int id, uint16_t offset,
					uint8_t opcode, struct bt_att *att,
					void *user_data)
{
	...
	//printf("CUSTOM Device Read called\n");
...
static void custom_service(struct server *server)
{
	int ret;
	bt_uuid_t uuid;
	char str[128]={0};
	struct gatt_db_attribute *service, *svc_chngd, *bb;

	bt_uuid16_create(&uuid, CUSTOM_SERVICE_UUID);

	service = gatt_db_add_service(server->db, &uuid, true, 4);

	bt_uuid16_create(&uuid, CUSTOM_CHARACTERISTIC_UUID);
	bb = gatt_db_service_add_characteristic(service, &uuid,
			BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
			BT_GATT_CHRC_PROP_NOTIFY | BT_GATT_CHRC_PROP_INDICATE,
			custom_device_read_cb,
			NULL, server);
	server->hr_handle = gatt_db_attribute_get_handle(bb);
							...
							uint8_t src_type)
{
    ...
    // 新增：连接Socket并发送"init"命令
    if (send_socket_command("init") == 0) {
        printf("Successfully sent 'init' command \n");
    } else {
        printf("Failed 2 send 'init' command \n");
    }

	printf("listening on ATT channel. Waiting for connections\n");
...
int ble_pic_send_notification(uint8_t *data, uint16_t len)
{
	int ret = -1;
	struct server *server = g_server;
	if(server && server->hr_handle){
		ret = bt_gatt_server_send_notification(server->gatt,
						server->hr_handle,
						data, len, false);
	}
	return ret;
...
void *my_worker(void *arg) 
{
	...
	while(1) {
		...
		char* pathname = "/tmp/123.jpg";
		if(0==access("/tmp/send",F_OK)) {
			unlink("/tmp/send");
			...
			uint16_t pack_idx=0;
			data[0] = 0x0;
			data[4] = (uint8_t)((fileSize&0xff000000) >> 24);
			data[3] = (uint8_t)((fileSize&0x00ff0000) >> 16);
			data[2] = (uint8_t)((fileSize&0x0000ff00) >> 8);
			data[1] = (uint8_t)((fileSize&0x000000ff) >> 0);
			ble_pic_send_notification(data, 5);
			printf("totall size=%ld,total pack=%d,0x%x,0x%02x,0x%02x,0x%02x\n",fileSize,fileSize/PACK_LEN,data[1],data[2],data[3],data[4]);
			// printf("raw data: ");
			...
			while(remain>0){
				...
				data[0] = 0x01;
				data[1] = (uint8_t)((pack_idx & 0xff00) >> 8);
				data[2] = (uint8_t)(pack_idx & 0xff);
				long rlen = fread(data+3, 1, PACK_LEN, fr);
				int ll=rlen+3;
				ble_pic_send_notification(data, ll);
				remain -= rlen;
				//printf("sending size=%ld,pack_idx=%d,0x%02x,0x%02x,remain=%d\n",ll,pack_idx,data[1],data[2],remain);
				// printf("raw data: ");
				// for(int i=0; i<100; i++) printf("%02x ",data[i]);
				// printf("\n");
				usleep(10*1000);
			}
...
static int send_socket_command(const char *cmd)
{
    ...
    // 发送命令
    bytes_written = sendto(sockfd, cmd, strlen(cmd), 0,
...
int main(int argc, char *argv[])
{
	...
	pthread_t ble_adv_tid;
	


	if (hci_devba(dev_id, &src_addr) < 0) {
		printf("Adapter not available\n");
		return -1;
	}
	...
	g_server = server_create(fd, mtu, hr_visible);
	if (!g_server) {
		close(fd);
		return EXIT_FAILURE;
	}
	...
	if (mainloop_add_fd(fileno(stdin),
				...
				prompt_read_cb, g_server, NULL) < 0) {
	...
	pthread_create(&ble_adv_tid, NULL, my_worker, NULL);
